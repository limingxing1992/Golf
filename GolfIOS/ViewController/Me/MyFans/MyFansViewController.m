...
- (void)loadDataWithRet:(BOOL)ret{
    ...
    [ShareBusinessManager.userManager postMyFansListWithParameters:@{@"currentPage":[NSNumber numberWithInteger:_currentPage], @"pageSize":PAGESIZE}
                                                           success:^(id responObject) {
                                                               NSDictionary *dict = responObject;
                                                               NSArray *data = dict[@"data"];
                                                               weakself.count = [dict[@"count"] integerValue];
                                                               weakself.name = [NSString stringWithFormat:@"粉丝(%ld)",weakself.count];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MyFansTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"fansCell"];
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController{
    MyFansSearchViewController *resultVc = (MyFansSearchViewController *)searchController.searchResultsController;
...
- (UITableView *)listTable{
    if (!_listTable) {
        ...
        [_listTable registerClass:[MyFansTableViewCell class] forCellReuseIdentifier:@"fansCell"];
        GOLFWeakObj(self);
        //下拉刷新
...
- (UISearchController *)searchController{
    if (!_searchController) {
        GOLFWeakObj(self);
        MyFansSearchViewController *searchVc = [[MyFansSearchViewController alloc] init];
