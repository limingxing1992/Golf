...
#import "ScoreSegement.h"

@interface ScoreSegement ()
...
@implementation ScoreSegement

- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        self.backgroundColor = WHITECOLOR;
        [self createLayersWithTitles:@[@"总积分", @"周积分"]];//创建标题指示器
        [self createTap];//添加手势
...
- (void)menuTapped:(UITapGestureRecognizer *)sender{
    ...
    NSInteger tapIndex = touchPoint.x / (self.frame.size.width / 2);
    //界面处理
    if (tapIndex != _lastSelectIndex) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (_lastSelectIndex == 0) {
                _indicatorLayer.center = CGPointMake(SCREEN_WIDTH / 4, HEIGHT_CELL - 1);
            }else{
                _indicatorLayer.center = CGPointMake(SCREEN_WIDTH / 4 *3, HEIGHT_CELL - 1);
            }
        }];
...
- (UIView *)indicatorLayer{
    if (!_indicatorLayer) {
        ...
        _indicatorLayer.frame = CGRectMake(0, 0, 50, 2);
        _indicatorLayer.center = CGPointMake(SCREEN_WIDTH / 4, HEIGHT_CELL - 1);
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string position:(CGPoint)point {
    ...
    CGFloat sizeWidth = (size.width < (self.frame.size.width / 2) - 25) ? size.width : self.frame.size.width / 2 - 25;
