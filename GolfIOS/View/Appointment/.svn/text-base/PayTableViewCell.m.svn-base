...
//        self.contentView.userInteractionEnabled = YES;
        ...
        [GOLFNotificationCenter addObserver:self selector:@selector(cancelSelected:) name:@"payStyle" object:nil];
        [GOLFNotificationCenter addObserver:self selector:@selector(openPayStyleWith:) name:@"payNoti" object:nil];
    }
    return self;
...
- (void)setInfoWith:(NSString *)style{
    ...
    if ([style isEqualToString:@"支付宝"]) {
        self.payIv.image = IMAGE(@"classify57");
        self.paySelectedBtn.selected = YES;
    }else if ([style isEqualToString:@"微信"]){
        self.payIv.image = IMAGE(@"classify58");
    }else if ([style isEqualToString:@"银联"]){
        self.payNoticeLb.text = @"暂未开通，敬请期待";
        self.payIv.image = IMAGE(@"classify191");
...
- (void)paySelectAction{
    if (_paySelectedBtn.isSelected) {
        ...
        //选中
        [GOLFNotificationCenter postNotificationName:@"payStyle" object:nil userInfo:@{@"status":self.payTitleLb.text}];//发送通知取消其他cell选中
        if (self.block) {
...
- (UIButton *)paySelectedBtn{
    if (_paySelectedBtn == nil) {
        ...
        [_paySelectedBtn setBackgroundImage:IMAGE(@"classify59") forState:UIControlStateNormal];
        [_paySelectedBtn setBackgroundImage:IMAGE(@"classify60") forState:UIControlStateSelected];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(paySelectAction)];
