...
@interface STL_TimePickerView ()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation STL_TimePickerView
...
- (instancetype)initWithFrame:(CGRect)frame title:(NSString *)title ary:(NSArray *)ary{
    ...
    if (self) {
        ...
        [GOLFNotificationCenter addObserver:self selector:@selector(firstSelect) name:@"firstTimeSelect" object:nil];
    }
    return self;
...
//    _week = week;
    ...
    _time = [NSString stringWithFormat:@"%.2ld:00",time];
...
- (void)sureAction{
    if (self.delegate) {
        [self.delegate sureWithTime:_time];
    }
    if (self.delegate_1) {
        [self.delegate_1 sureWithTime:_time];
    }
}
